...
export function Hero() {
  ...
  // No renderizar hasta tener el idioma del cliente
  if (!lang) {
    return (
  ...
  return (
    <section id="top" className="relative overflow-hidden hero-glow">
      ...
      <div className="container-dm relative pt-32 md:pt-36">
        <h1 className="animate-fade-up font-display text-[16vw] font-bold leading-[0.9] tracking-tight text-white sm:text-[13vw] lg:text-[130px]">
          {t("hero.your_code")}
        </h1>
        ...
        <div className="relative mt-8 flex flex-col gap-10 md:mt-12 md:flex-row md:items-start md:justify-between">
          <ul className="animate-fade-up space-y-3 text-lg text-white/85 [animation-delay:120ms] md:text-xl">
            {[t("hero.list_development"), t("hero.list_training"), t("hero.list_formation")].map((item) => (
          ...
          <div className="bubble animate-fade-up w-full max-w-sm p-6 [animation-delay:220ms] md:mt-2">
            ...
            <p className="text-lg font-semibold leading-snug text-white">{t("hero.tagline")}</p>
          </div>
        </div>

        <div className="relative z-10 -mt-2 md:-mt-16">
          <img src="/laptop.png" alt="Klyro" className="mx-auto w-full max-w-[680px] animate-float drop-shadow-[0_40px_80px_rgba(0,0,0,0.6)]" />
        </div>

        <div className="relative z-20 md:-mt-24 lg:-mt-32">
          <div className="bubble animate-fade-up max-w-md p-6 [animation-delay:120ms]">
            <p className="text-lg font-medium leading-snug text-white/90">{t("hero.subtitle")}</p>
            <a href="/cotiza" className="group mt-5 inline-flex items-center gap-2 rounded-full bg-primary px-6 py-3 text-sm font-semibold text-white shadow-[0_10px_30px_-8px_rgba(31,218,113,0.7)] transition-transform hover:scale-[1.03] active:scale-95">
              {t("hero.cta")}<ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
            </a>
          </div>
        </div>
